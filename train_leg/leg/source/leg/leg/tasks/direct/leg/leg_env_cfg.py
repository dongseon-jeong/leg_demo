...
class LegEnvCfg(DirectRLEnvCfg):
    ...
    rew_scale_forward_vel = 20.0     #25 앞으로 가면 많이 보상
    rew_vel_track_rate = 8.0 # 5 속도 증가
    rew_heading_rate = -15.0  #  1.0

    # penalties는 일단 아주 약하게 시작
    rew_scale_joint_vel = -0.01
    rew_scale_action_rate = -0.1
    rew_scale_energy = -0.0001          # 처음엔 꺼버려도 됨
    rew_leg_pose_rate = -0.1
    rew_lateral_vel_rate = -50.0 # 0.5
    rew_vertical_vel_rate = -2.0 # 0.2
    rew_body_ang_vel_rate = -0.5

    # 스텝용
    foot_target_width = 0.06
    ground_z = 0.01
    swing_z = 0.08
    rew_fheight_rate = 1.0 #0.25
    rew_foot_flat_rate = 1.0 #0.5
